...
def visualize_positions(positions, colors, titles, fig_title, parents, change_color_after_frame=None, overlay=False,
    ...
    # plt.show() and plt.close() can only be used locally, to use on the cluster, use line_ani.save() 
    # plt.show()
    # plt.close()

    if not os.path.exists('./animations'):
        os.mkdir('animations')
    line_ani.save('animations/{}.gif'.format(fig_title), writer='pillow', dpi=100)
    print("Saved {}".format(fig_title))
